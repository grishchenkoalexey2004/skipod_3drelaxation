...
void relax()
{
    ...
    #pragma omp parallel num_threads(c_num_threads)
	{
		...
		for (int newi=1;newi<=cur_dim-2+c_num_threads-1;newi++){
			int i = newi-iam;
		...
		for (int newj=1;newj<=cur_dim-2+c_num_threads-1;newj++){
			int j = newj-iam;
		...
		for(int newk=1; newk<=cur_dim-2+c_num_threads-1; newk++){
			int k = newk-iam;
